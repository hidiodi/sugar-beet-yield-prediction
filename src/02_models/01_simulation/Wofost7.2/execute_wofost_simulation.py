...
# REFACTORED (v22.1): Bio-Physical Risk Scanner & Test Mode Fix
# Description: Runs WOFOST with "Mechanism-Informed" metrics:
#              - Oxygen Stress (Anoxia) derived from Soil State (not just rain)
#              - Sowing Failure derived from the Initial Conditions input
#              - Terminal Freeze derived from Fall Temperatures
...
def _run_single_forecast_member(district_no, member, weather_for_member_df, crop_params, static_site_row,
    ...
    try:
        ...
        # --- 3. Bio-Physical Risk Scanner (Mechanism-Informed) ---
        ...
        # B. Hydraulic Risk: Oxygen Stress / Anoxia (Soil State Based)
        # We define stress as Soil Moisture (SM) >= Field Capacity (SMFCF)
        # during the peak growth months (June, July, August)
        oxygen_stress_days = 0
        if not output_wlp.empty and 'SM' in output_wlp.columns:
            sm_threshold = static_site_row['SMFCF']
            summer_mask_sim = (output_wlp.index.month.isin([6, 7, 8]))
            # Count days where soil is wetter than Field Capacity
            oxygen_stress_days = output_wlp[summer_mask_sim & (output_wlp['SM'] >= sm_threshold)].shape[0]
        ...
        return {
            ...
            'oxygen_stress_days': oxygen_stress_days,
...
def run_forecast_simulation(df_static_year, df_forecast_weather_year, df_initial_conditions_year, crop_params):
    ...
    year = df_static_year['year'].iloc[0]
    # Set backend to 'loky' for reliability
...
def aggregate_and_save_extreme_weather_metrics(df_fcst_ensemble, output_path):
    ...
    # NEW Aggregations matching the internal logic
    aggs = {
        'simulation_failed': ['mean'],
        'cumulative_water_stress': ['mean', 'std', p90],
        'spring_mud_days': ['mean'],
        'oxygen_stress_days': ['mean'],
        'terminal_frost_occurrence': ['mean'],
        'harvest_respiration_gdd': ['mean'],
        # KEEP RAW YIELD FOR FEATURE SET
        'yield_water_limited': ['mean']
    }

    # Apply only to existing columns (Safe Aggregation)
    ...
    # Rename for Final CSV consistency with Feature Engineering
    rename_map = {
        'spring_mud_days_mean': 'prob_sowing_failure',
        'oxygen_stress_days_mean': 'anoxia_events',
        'terminal_frost_occurrence_mean': 'prob_terminal_freeze',
        'harvest_respiration_gdd_mean': 'harvest_respiration_risk',
        'yield_water_limited_mean': 'wofost_yield_water_limited'
    }
...
def main():
    ...
    output_dir.mkdir(exist_ok=True, parents=True)  # Ensure parents exist

    try:
        logging.info("Loading static assets...")
        df_static_all = pd.read_csv(PROCESSED_DATA_DIR / 'StaticSiteData.csv', dtype={'district_no': str})

        # --- TEST MODE LOGIC (Added v22) ---
        limit = CONFIG.get('DISTRICT_LIMIT')
        if limit:
            logging.warning(f"!!! TEST MODE: Limiting simulation to top {limit} districts !!!")
            target_districts = sorted(df_static_all['district_no'].unique())[:limit]
            df_static_all = df_static_all[df_static_all['district_no'].isin(target_districts)]
        # -----------------------------------
    ...
    # Filter dataframes based on valid tasks
    ...
    if all_fcst_results:
        ...
        # Save Metrics to the EXACT location defined in Config
        metrics_path = Path(CONFIG['FILE_PATHS']['EXTREME_WEATHER_METRICS_OUTPUT'])
        aggregate_and_save_extreme_weather_metrics(final_fcst, metrics_path)
