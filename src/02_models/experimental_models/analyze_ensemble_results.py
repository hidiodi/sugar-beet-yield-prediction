# File: src/visualization/analyze_ensemble_results.py
# Description: Loads the results from the raw Deep Ensemble backtest, recalculates key metrics,
#              and generates the standard diagnostic plots for analysis and comparison.
...
# --- Configuration ---
# Point this to the results file from your last run
RESULTS_CSV_PATH = 'reports/figures/district_level_diagnostics/deep_ensemble_champion/ensemble_backtest_results.csv'
GEOJSON_PATH = os.path.join('data', '01_raw', 'districts_official.geojson')
# Save the new plots in the same directory for consistency
REPORT_DIR = 'reports/figures/district_level_diagnostics/deep_ensemble_champion'
...
def print_summary_metrics(results_df: pd.DataFrame):
    ...
    print("--- Recalculating Performance Summary for Deep Ensemble ---")

    # Ensure columns exist before calculating
    if 'predicted_yield_lower' not in results_df.columns:
        print("❌ Error: Results file is missing required prediction columns.")
        return
    ...
    avg_aleatoric = results_df['aleatoric_uncertainty'].mean()
    ...
    print(f"Average Aleatoric Uncertainty (Data Noise): {avg_aleatoric:.2f} dt/ha")
...
def plot_national_average_timeline(backtest_results: pd.DataFrame):
    ...
    plt.plot(yearly_avg['year'], yearly_avg['avg_pred_median'], label='Median Prediction (Ensemble Mean)', color='red',
             linestyle='--', zorder=4)
    plt.fill_between(yearly_avg['year'], yearly_avg['avg_pred_lower'], yearly_avg['avg_pred_upper'], color='blue',
                     alpha=0.2, label='Raw Ensemble Interval', zorder=2)
    plt.title("National Average Yield vs. Predicted Interval (Raw Deep Ensemble)", fontsize=16)
    plt.xlabel("Year");
...
def plot_best_worst_district_timelines(district_performance: pd.DataFrame, backtest_results: pd.DataFrame):
    ...
    for i, (_, district_info) in enumerate(districts_to_plot.iterrows()):
        ...
        ax.plot(data['year'], data['predicted_yield_median'], label='Median Prediction', color='red', linestyle='--',
                zorder=4)
        ax.fill_between(data['year'], data['predicted_yield_lower'], data['predicted_yield_upper'], color='blue',
                        alpha=0.2, label='Raw Ensemble Interval')
        ax.set_title(f"{'Best' if i < 3 else 'Worst'}: {district_info['name']}\n(R² = {district_info['r2']:.2f})")
        ax.legend();
        ax.grid(True, linestyle=':')
    plt.suptitle("Prediction Timelines for Best & Worst Districts (Raw Deep Ensemble)", fontsize=18)
...
def plot_performance_map(district_performance: pd.DataFrame, gdf_districts: gpd.GeoDataFrame):
    ...
    # Handle districts that exist in the GeoJSON but had no data in the backtest
    ...
    ax.set_title('Model Performance (R²) by District - Raw Deep Ensemble', fontsize=16)
    ax.set_axis_off()
...
def main():
    ...
    print(f"--- Generating Additional Analysis for Deep Ensemble ---")
    ...
    print(f"\n--- Analysis Complete ---")
    print(f"Standard diagnostic plots have been saved to: {REPORT_DIR}")
