...
def _create_district_specific_parameters(static_row, cropdata):
    ...
    try:
        sand = static_row[soil_map['sand']] / 100.0; clay = static_row[soil_map['clay']] / 100.0
        som = static_row[soil_map['som']] / 100.0; bdod = static_row[soil_map['bdod']]
        ...
        logging.info(f"  (For comparison, Generic Loam was: SMW=0.100, SMFCF=0.300, SM0=0.400)")
        logging.info("-" * 70)
...
def load_data_and_setup_model(year, cfg):
    ...
    gdf_districts_proj = gdf_districts.to_crs(epsg=3035)
    centroids_wgs84 = gdf_districts_proj.geometry.centroid.to_crs(epsg=4326)
    ...
    if cfg['DISTRICT_LIMIT'] is not None:
        logging.warning(f"[QUICK TEST] Limiting run to the first {cfg['DISTRICT_LIMIT']} districts.")
    ...
    try:
        ...
        add_params(cropdata, generic_data); add_params(cropdata, crop_params[variety_name])
...
def run_historical_simulation(df_static, df_daily_hist, cropdata, year, cfg):
    ...
    if df_daily_hist_year.empty: logging.error(f"[HISTORICAL] FATAL: No weather data for {year}!"); return pd.DataFrame()
    results = []
    for _, row in tqdm(df_static.iterrows(), total=len(df_static), desc="Historical Sim"):
        district_no = row['district_no']; weather_df = df_daily_hist_year[df_daily_hist_year['district_no'] == district_no].copy()
        if weather_df.empty: logging.warning(f"[HISTORICAL] No weather for district {district_no}"); continue
        try:
            ...
            output = model.get_output(); simulated_yield = output[-1]['TWSO'] if output else np.nan
        except Exception as e:
            logging.error(f"[HISTORICAL] ERROR for district {district_no}: {e}", exc_info=True); simulated_yield = np.nan
        results.append({'year': year, 'district_no': district_no, 'actual_yield': row['kreisYield'], 'lintul_yield_perfect_weather': simulated_yield})
...
def _run_single_forecast_member(member_row, district_no, year, wg, parameters, site_data, cfg):
    try:
        ...
        logging.error(f"[FORECAST_WORKER] Error for dist {district_no}, member {member_row.get('member', 'N/A')}: {e}"); return np.nan
...
def run_forecast_simulation(df_static, df_daily_hist, df_seas5, cropdata, year, cfg):
    ...
    district_params = {row['district_no']: _create_district_specific_parameters(row, cropdata) for _, row in df_static.iterrows()}
    ...
    for district_no, group in tqdm(df_seas5.groupby('district_no'), desc="Forecast Sim (Districts)"):
        ...
        tasks = [delayed(_run_single_forecast_member)(member_row, district_no, year, wg, parameters, site_data, cfg) for _, member_row in group.iterrows()]
...
if __name__ == "__main__":
    ...
    if cropdata is not None:
        ...
        for _, row in df_static.iterrows():
            try: _create_district_specific_parameters(row, cropdata)
