# File: src/models/backtest_hybrid_cqr_model.py
# Description: Backtesting and diagnostics for the Hybrid Adaptive CQR model.
#              This method trains on all available past data, then calibrates the
#              adjustment factor on the most recent slice of that training data.
...
# --- Configuration for the Hybrid Adaptive CQR Model ---
...
REPORT_DIR = os.path.join('reports', 'figures', 'district_level_diagnostics', 'hybrid_adaptive_cqr_champion')

BACKTEST_START_YEAR = 2000
BACKTEST_END_YEAR = 2024
CALIBRATION_WINDOW_SIZE = 3  # Years of recent *training* data to use for each fold's calibration
TARGET_COVERAGE = 0.95
LOW_DATA_THRESHOLD = 10
MIN_DATAPOINTS_FOR_PLOT = 10


def run_hybrid_cqr_backtest(df: pd.DataFrame, feature_cols: list, model_lower_clone: XGBRegressor,
                            model_median_clone: XGBRegressor, model_upper_clone: XGBRegressor):
    """
    Performs a rolling forecast using the hybrid adaptive CQR approach.
    1. Trains models on all data < prediction_year.
    2. Calibrates q_adj on the last few years of that training data.
    3. Predicts on prediction_year.
    """
    print(f"\n--- Starting Hybrid Adaptive CQR Backtest from {BACKTEST_START_YEAR} to {BACKTEST_END_YEAR} ---")
    all_predictions = []

    start_year_for_backtest = df['year'].min() + CALIBRATION_WINDOW_SIZE
    if BACKTEST_START_YEAR < start_year_for_backtest:
        print(f"Adjusting backtest start year to {start_year_for_backtest} to allow for initial calibration window.")

    for year_to_predict in tqdm(range(max(BACKTEST_START_YEAR, start_year_for_backtest), BACKTEST_END_YEAR + 1),
                                desc="Backtesting Years"):

        # --- Data Splitting for this Fold ---
        # The training set includes ALL data prior to the year we want to predict.
        ...
        # The calibration set is the most recent slice OF THE TRAINING SET.
        calibration_start_year = year_to_predict - CALIBRATION_WINDOW_SIZE
        calib_df = train_df[train_df['year'] >= calibration_start_year].copy()

        if calib_df.empty:
            # print(f"Skipping year {year_to_predict}, not enough recent data for calibration.")
            continue

        # --- 1. Model Training on FULL historical data ---
        ...
        # --- 2. Adaptive Calibration on RECENT slice of training data ---
        ...
        # --- 3. Prediction on Test Set with Fold-Specific Adjustment ---
    ...
    print("\nHybrid Adaptive CQR backtest complete.")
    return results_df


def analyze_interval_performance(results_df: pd.DataFrame):
    """Analyzes the performance of the hybrid adaptive CQR interval."""
    print("\n--- Analyzing Hybrid Adaptive Prediction Interval Performance (Target: 95%) ---")
    ...
    print(f"Hybrid Adaptive Prediction Interval Coverage (PICP): {coverage:.2%}")
...
# --- PLOTTING FUNCTIONS (Modified for Hybrid Adaptive CQR context) ---

def plot_national_average_timeline(backtest_results: pd.DataFrame, report_dir: str):
    """Generates a national average plot with the 95% hybrid adaptive CQR interval."""
    ...
    plt.plot(yearly_avg['year'], yearly_avg['avg_pred_median'], label='Median Prediction', color='red', linestyle='--',
             zorder=4)
    plt.fill_between(yearly_avg['year'], yearly_avg['avg_pred_lower'], yearly_avg['avg_pred_upper'], color='darkorange',
                     alpha=0.25, label='95% Hybrid Adaptive Interval', zorder=2)
    plt.title("National Average Yield vs. Predicted Interval (Hybrid Adaptive CQR)", fontsize=16)
    plt.xlabel("Year");
    plt.ylabel("Yield (dt/ha)")
    plt.legend();
    plt.grid(True, linestyle=':')
    plt.savefig(os.path.join(report_dir, '01_hybrid_cqr_national_average_timeline.png'), bbox_inches='tight')
    plt.close()
...
def plot_best_worst_district_timelines(district_performance: pd.DataFrame, backtest_results: pd.DataFrame,
    ...
    for i, (_, district_info) in enumerate(districts_to_plot.iterrows()):
        ...
        ax.plot(data['year'], data['predicted_yield_median'], label='Median Prediction', color='red', linestyle='--',
                zorder=4)
        ax.fill_between(data['year'], data['predicted_yield_lower'], data['predicted_yield_upper'], color='darkorange',
                        alpha=0.25, label='95% Hybrid Interval')
        ax.set_title(f"{'Best' if i < 3 else 'Worst'}: {district_info['name']}\n(R² = {district_info['r2']:.2f})")
        ax.legend();
        ax.grid(True, linestyle=':')
    plt.suptitle(f"Prediction Timelines for Best & Worst Districts (Hybrid Adaptive CQR)", fontsize=18)
    plt.tight_layout(rect=[0, 0, 1, 0.96])
    plt.savefig(os.path.join(report_dir, '02_hybrid_cqr_best_worst_districts.png'), bbox_inches='tight')
    plt.close()
...
def plot_performance_map(district_performance: pd.DataFrame, gdf_districts: gpd.GeoDataFrame, report_dir: str):
    ...
    ax.set_title('Model Performance (R²) by District - Hybrid Adaptive CQR', fontsize=16);
    ax.set_axis_off()
    plt.savefig(os.path.join(report_dir, '03_hybrid_cqr_r_squared_map.png'), bbox_inches='tight');
    plt.close()
...
def plot_r2_vs_data_count(district_performance: pd.DataFrame, report_dir: str):
    ...
    plt.title("Data Availability vs Model Performance - Hybrid Adaptive CQR")
    plt.xlabel("Number of Years in Backtest per District");
    plt.ylabel("R-squared (R²)")
    plt.legend(title='Is Low Data?')
    plt.savefig(os.path.join(report_dir, '04_hybrid_cqr_r2_vs_data_count.png'), bbox_inches='tight');
    plt.close()


def main():
    """Main function to orchestrate the Hybrid Adaptive CQR model evaluation."""
    os.makedirs(REPORT_DIR, exist_ok=True)
    print("--- Starting Hybrid Adaptive CQR Model Evaluation Pipeline ---")
    ...
    backtest_results = run_hybrid_cqr_backtest(df, feature_cols, model_lower, model_median, model_upper)
