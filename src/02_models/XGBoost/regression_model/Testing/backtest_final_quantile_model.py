...
#                            clean feature set for the backtest loop.
...
def run_backtest_with_cqr(df: pd.DataFrame, full_feature_list: list, model_lower_clone: XGBRegressor,
    ...
    # This prevents target leakage and KeyErrors from missing/renamed columns.
    ...
    for year_to_predict in tqdm(range(BACKTEST_CONFIG['BACKTEST_START_YEAR'], BACKTEST_CONFIG['BACKTEST_END_YEAR'] + 1),
        ...
        # Use the clean feature list for all data splits
        X_train, y_train_residual = train_df[actual_training_features], train_df['forecast_residual']
        X_calib, y_calib_residual = calib_df[actual_training_features], calib_df['forecast_residual']
        ...
        # Use the original, consistent column name for prediction
        fold_results['predicted_yield_lower'] = test_df[wofost_col] + predicted_residual_lower - q_hat
        fold_results['predicted_yield_median'] = test_df[wofost_col] + predicted_residual_median
        fold_results['predicted_yield_upper'] = test_df[wofost_col] + predicted_residual_upper + q_hat
...
# ... All plotting and analysis functions remain unchanged ...
...
def plot_national_average_timeline(backtest_results: pd.DataFrame, report_dir: str):
    ...
    plt.title("National Average Yield vs. Predicted Interval (Final Quantile Model)", fontsize=16);
    plt.xlabel("Year");
...
def plot_best_worst_district_timelines(district_performance: pd.DataFrame, backtest_results: pd.DataFrame,
    ...
    if len(reliable_perf) < 6: print(
        f"   Warning: Not enough reliable districts (found {len(reliable_perf)}) to plot. Skipping."); return
    ...
    plt.suptitle(f"Prediction Timelines for Best & Worst Districts (Final Quantile Model)", fontsize=18);
...
def plot_performance_map(district_performance: pd.DataFrame, gdf_districts: gpd.GeoDataFrame, report_dir: str):
    ...
    ax.set_title('Model Performance (R²) by District - Final Quantile Model', fontsize=16);
    ax.set_axis_off();
...
def plot_r2_vs_data_count(district_performance: pd.DataFrame, report_dir: str):
    ...
    plt.title("Data Availability vs Model Performance - Final Quantile Model")
...
def main():
    ...
    # --- START OF FIX: Use consistent column names ---
    wofost_col = 'wofost_forecast_yield_fresh_dt'
    ...
    feature_cols_from_config = XGB_CONFIG['FEATURE_COLS']

    backtest_results = run_backtest_with_cqr(df, feature_cols_from_config, model_lower, model_median, model_upper)
    ...
    reliable_districts = district_performance[
        district_performance['data_point_count'] >= BACKTEST_CONFIG['LOW_DATA_THRESHOLD']]
    if not reliable_districts.empty:
        reliable_backtest_results = backtest_results[
            backtest_results['district_no'].isin(reliable_districts['district_no'])]
        r2_reliable = r2_score(reliable_backtest_results['kreisYield'],
                               reliable_backtest_results['predicted_yield_median'])
