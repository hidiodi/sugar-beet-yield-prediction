# File: src/02_models/Wofost7.1/wofost_modeling/sowing.py
...
class DynamicSowingManager:
    ...
    def __init__(self, sowing_window_start_month=3, sowing_window_start_day=15,
        ...
        :param temp_threshold_c: The moving average temperature that must be
                                 exceeded.
        :param temp_avg_period_days: The number of days for the temperature
                                     moving average.
        """
    ...
    def find_sowing_date(self, weather_df_year: pd.DataFrame) -> datetime.date:
        ...
        :param weather_df_year: DataFrame with daily weather for the entire
                                year. Must contain 'date', 'tmin', 'tmax'.
        :return: The calculated sowing date as a datetime.date object.
        """
        df = weather_df_year.copy()
        df['mean_temp'] = (df['tmin'] + df['tmax']) / 2
        df['temp_ma'] = df['mean_temp'].rolling(
            window=self.period, min_periods=self.period).mean()

        try:
            year = df['date'].dt.year.iloc[0]
            window_start = datetime.date(
                year, self.start_month, self.start_day)
            window_end = datetime.date(year, self.end_month, self.end_day)
        except IndexError:
            logging.error(
                "DynamicSowingManager: weather_df_year is empty. "
                "Cannot determine year.")
            return datetime.date(2000, self.start_month, self.start_day)
        ...
        if not potential_sow_days.empty:
            ...
            logging.debug(
                f"Dynamic sowing date found: {sowing_date} "
                f"(Threshold {self.threshold}C met).")
            return sowing_date
        else:
            # Fallback: if no day meets the criteria, sow on the last day of
            # the window.
            logging.debug(
                f"No suitable day found. Sowing on fallback date: {window_end}.")
            return window_end
