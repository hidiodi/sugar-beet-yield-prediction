...
if __name__ == "__main__":
    ...
    if pcse_log_dir.exists():
        try:
            for f in pcse_log_dir.glob("pcse.log*"): os.remove(f)
    ...
    # --- 2. CONSOLIDATED DATA LOADING (V2 WORKFLOW) ---
    logging.info("=" * 70 + "\nLoading and merging ALL data sources (V2 WORKFLOW)...\n" + "=" * 70)
    try:
        # Load all base dataframes
        ...
        # Use .copy() to avoid SettingWithCopyWarning later if changes are made to gdf_districts
        gdf_districts_coords = gdf_districts[['id', 'geometry']].copy()
        # UserWarning about geographic CRS is expected and can be ignored for our purpose here
        ...
        # Merge into a single master static/dynamic dataframe
        ...
        # Validate and handle missing WAV
        missing_wav = df_static_all['WAV'].isna()
        if missing_wav.any():
            logging.warning(f"{missing_wav.sum()} rows have missing WAV values. Filling with default 10.0.")
            df_static_all.loc[missing_wav, 'WAV'] = 10.0

        # Filter out rows with zero or NaN yield before further processing
        ...
        if len(df_static_all) < initial_rows:
            logging.warning(
                f"Removed {initial_rows - len(df_static_all)} rows with missing or zero actual_yield values.")

        # Load and synchronize forecast data
        ...
        logging.info(f"Proceeding with {len(df_static_all)} valid district-year records for simulation.")
    ...
    logging.info(
        f"Loaded {len(full_hist_weather_df)} total historical weather records from {weather_load_start_year} to {CONFIG['END_YEAR']}.")

    logging.info("Loading and parsing crop YAML file...")
    try:
        ...
        for key, val in p.items():
            ...
            elif key not in ['Metadata', '<<']:  # Handle direct values like DVSEND if not a list
                cropdata.add_variable(key, val)
        logging.info(
            f"Successfully loaded and merged parameters for variety: Sugarbeet_601. Test: TSUM1={cropdata.get('TSUM1', 'N/A')}")
    except Exception as e:
        logging.error(f"FATAL: Could not load or parse crop YAML. Error: {e}", exc_info=True);
        sys.exit(1)

    # --- 4. RUN V2 INPUT VALIDATION ---
    # This must be called *after* all data is loaded and merged into df_static_all
    ...
    for year in range(CONFIG['START_YEAR'], CONFIG['END_YEAR'] + 1):
        ...
        if df_static_year.empty:
            logging.warning(
                f"Missing static data for year {year} (after applying DISTRICT_LIMIT if any). Skipping simulation for this year.");
            continue
        if df_seas5_year.empty:  # Only critical for forecast simulation, but good to check early
            logging.warning(
                f"Missing forecast data for year {year}. Forecast simulations will be skipped for this year.");

        # --- Weather Generator fitting for analog year selection (original complex logic) ---
    ...
    if all_hist_results and all_fcst_results:
        ...
        # Calling original analysis functions
