...
#              final NGBoost model. This version includes proper data handling
#              and validation sets inside the backtesting loop to ensure a robust evaluation.
...
# --- Configuration ---
MODEL_PATH = os.path.join('src/models', 'final_ngboost_model.joblib')
...
REPORT_DIR = os.path.join('reports', 'figures', 'district_level_diagnostics', 'final_ngboost_champion')
...
def run_backtest_with_ngboost(df: pd.DataFrame, feature_cols: list, model_clone: NGBRegressor):
    """Performs a rolling forecast for the NGBoost model with proper validation."""
    print(f"\n--- Starting Backtest with NGBoost from {BACKTEST_START_YEAR} to {BACKTEST_END_YEAR} ---")
    all_predictions = []
    ...
    for year_to_predict in tqdm(range(BACKTEST_START_YEAR, BACKTEST_END_YEAR + 1), desc="Backtesting Years"):
        ...
        # Create a validation set within each backtest fold
        ...
        X_train, y_train = train_df[feature_cols], train_df['forecast_residual']
        X_val, y_val = val_df[feature_cols], val_df['forecast_residual']
        ...
        lower_bounds = pred_dist.ppf(0.5 - NOMINAL_COVERAGE / 2)
        upper_bounds = pred_dist.ppf(0.5 + NOMINAL_COVERAGE / 2)
        median_preds = pred_dist.ppf(0.5)

        fold_results = test_df[['district_no', 'year', 'kreisYield', 'name', 'stage1_forecast']].copy()
        fold_results['predicted_yield_lower'] = test_df['stage1_forecast'] + lower_bounds
        fold_results['predicted_yield_median'] = test_df['stage1_forecast'] + median_preds
        fold_results['predicted_yield_upper'] = test_df['stage1_forecast'] + upper_bounds
...
def plot_performance_map(district_performance: pd.DataFrame, gdf_districts: gpd.GeoDataFrame, report_dir: str):
    ...
    ax.set_title('Model Performance (R²) by District - NGBoost Model', fontsize=16)
    ax.set_axis_off()
...
def plot_r2_vs_data_count(district_performance: pd.DataFrame, report_dir: str):
    ...
    plt.title("Data Availability vs Model Performance - NGBoost Model")
...
def main():
    ...
    feature_cols = [
        ...
        'summer_precip_prob_wet_forecast', 'lat', 'lon', 'avg_elevation', 'avg_slope', 'avg_bdod_0_30cm',
        ...
        'winter_cropland_LST_anomaly', 'winter_cropland_snow_cover_days', 'profit_margin_proxy_lag1',
        'cost_of_inputs_lag1',
    ...
    backtest_results = run_backtest_with_ngboost(df, feature_cols, ngb_model)
