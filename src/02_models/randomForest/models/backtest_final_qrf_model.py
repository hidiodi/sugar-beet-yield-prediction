# File: src/models/backtest_final_qrf_model.py
# Description: Backtesting and diagnostics for the Quantile Regression Forest (QRF) model.
...
# --- Configuration for the Final QRF Model ---
MODEL_PATH = os.path.join('src/models', 'final_qrf_model.joblib')  # <-- CHANGED
...
REPORT_DIR = os.path.join('reports', 'figures', 'district_level_diagnostics', 'final_qrf_champion')
...
def run_backtest(df: pd.DataFrame, feature_cols: list, model_base_clone: RandomForestQuantileRegressor):  # <-- CHANGED
    """Performs a rolling forecast using the QRF's built-in quantile prediction."""
    print(f"\n--- Starting QRF Backtest from {BACKTEST_START_YEAR} to {BACKTEST_END_YEAR} ---")
    all_predictions = []
    ...
    for year_to_predict in tqdm(range(BACKTEST_START_YEAR, BACKTEST_END_YEAR + 1), desc="Backtesting Years"):
        ...
        # --- KEY CHANGE: Train one model, then get quantiles directly ---
        model = clone(model_base_clone)
        model.fit(X_train, y_train)

        # The model's predict method does all the work for us!
        # It returns a numpy array of shape [n_samples, n_quantiles]
        quantiles_to_predict = [0.025, 0.5, 0.975]
        q_preds = model.predict(X_test, quantiles=quantiles_to_predict)
...
def plot_national_average_timeline(backtest_results: pd.DataFrame, report_dir: str):
    ...
    plt.title("National Average Yield vs. Predicted Interval (Final QRF Model)", fontsize=16)  # <-- Title Changed
    plt.xlabel("Year");
...
def plot_best_worst_district_timelines(district_performance: pd.DataFrame, backtest_results: pd.DataFrame,
    ...
    plt.suptitle(f"Prediction Timelines for Best & Worst Districts (Final QRF Model)", fontsize=18)  # <-- Title Changed
...
def plot_performance_map(district_performance: pd.DataFrame, gdf_districts: gpd.GeoDataFrame, report_dir: str):
    ...
    ax.set_title('Model Performance (R²) by District - Final QRF Model', fontsize=16);  # <-- Title Changed
    ax.set_axis_off()
...
def plot_r2_vs_data_count(district_performance: pd.DataFrame, report_dir: str):
    ...
    plt.title("Data Availability vs Model Performance - Final QRF Model")  # <-- Title Changed
...
def main():
    """Main function to orchestrate the Final QRF model evaluation."""
    os.makedirs(REPORT_DIR, exist_ok=True)
    print("--- Starting Final Quantile Regression Forest (QRF) Model Evaluation Pipeline ---")

    try:
        # --- KEY CHANGE: Load one model ---
    ...
    # --- KEY CHANGE: Pass the single model to the backtest function ---
